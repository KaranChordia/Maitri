...
Keep approved interior previews inside the originating Library book card rather than opening a separate preview panel. Show the portrait cover at rest, then reveal up to two wide approved pages with page number and title inside the same media footprint on hover or keyboard focus; provide tap-to-toggle on touch devices. Do not auto-scroll, open a nested reader, add separate controls, invent pages, or show body copy. Cover-only titles stay cover-only and non-interactive, and reduced-motion users receive the same content without transition effects.
...
Content Studio lives at `/Maitri/content-studio/index.html` (`public/content-studio/`). Link it from the site footer as `content-studio/index.html` (not a bare directory path — Vite SPA fallback would otherwise serve the homepage). Do not add it to the primary header navigation. Keep drafts claim-safe (no unsupported safety, materials, compliance, production, or preorder timeline language) and reuse the live motif mapping above.
